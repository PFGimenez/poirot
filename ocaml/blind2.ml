...
let search (fuzzer: grammar -> part list) (oracle: part list -> bool) (g: grammar) (goal: element) (max_depth: int) (injection_tokens: element list) : ext_grammar option =
    ...
    let compare_with_score (a: int * int * ext_element) (b: int * int * ext_element) : int = match a,b with
        ...
        | (ag,ah,_),(bg,bh,_) when ag=bg && bh=bh -> 0
        | _ -> 1 in

    let add_in_list g (openset: (int * int * ext_element) list) (new_elems: ext_element list) : (int * int * ext_element) list =
        List.merge compare_with_score (List.sort compare_with_score (List.map (fun (e: ext_element) : (int * int * ext_element) -> (g,Hashtbl.find distance_to_goal (element_of_ext_element e, goal),e)) new_elems)) openset in
    ...
    let rec search_aux (visited: (ext_element, bool) Hashtbl.t) (step: int) : (int * int * ext_element) list -> ext_grammar option = function
    | [] -> None
    | (distance,_,t)::q ->
