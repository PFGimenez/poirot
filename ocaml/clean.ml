...
let rec iterate_until_convergence (f : ext_grammar -> ext_rules) (g : ext_grammar) : ext_grammar = let old_length = List.length g.rules and new_rules = f g in
...
let clean_once (g : ext_grammar) : ext_rules = ((* remove_epsilon_symbols *) (remove_unreachable_symbols (remove_useless_symbols g))).rules
