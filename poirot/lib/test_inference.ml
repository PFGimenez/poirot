...
let ()=
    ...
    let parenth_oracle (prefix: part) (suffix: part) (inj: part option) : Oracle.oracle_status =
        ...
        match inj with
        | None -> print_endline "No injection"; Syntax_error
        | Some i -> print_endline ("Verify: "^(string_of_part i)); parenth_oracle_aux (prefix@i@suffix) [] in
...
(*    let oracle_mem2 (o: part -> Oracle.oracle_status) : part option -> Oracle.oracle_status =
    ...
    let speclist = [
        ("-grammar",    Arg.String (fun s -> grammar := Some (Grammar_io.read_bnf_grammar false s)),     "Target grammar");
        ("-pf",         Arg.String (fun s -> prefix := Some (Grammar_io.read_tokens false s)),     "Prefix of the request");
        ("-sf",         Arg.String (fun s -> suffix := Some (Grammar_io.read_tokens false s)),     "Suffix of the request");
        ("-goal",       Arg.String (fun s -> goal := Some (Grammar_io.read_token false s)),     "Terminal or nonterminal to reach");
        ("-start",      Arg.String (fun s -> start := Some (Grammar_io.read_tokens false s)),     "A valid injection, either terminal or nonterminal");
    ...
    if !grammar <> None && !prefix <> None && !suffix <> None && !goal <> None then begin
        ...
        Grammar_io.export_antlr4 "test" grammar;

(*        let oracle = oracle_mem2 (Fuzzer.oracle prefix suffix grammar) in*)
        let oracle = parenth_oracle prefix suffix in
        let fuzzer = Tree_fuzzer.fuzzer 0 None (Some goal) in
