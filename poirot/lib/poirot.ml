...
let search ?(subst: (element,string) Hashtbl.t option = None) ?(max_depth: int = 10) ?(forbidden_chars: char list = []) ?(sgraph_fname: string option = None) ?(qgraph_fname: string option = None) (oracle_fname: string)  (g: grammar) (goal: element) (start: element list option) : grammar option =
    ...
    Option.map Grammar.grammar_of_ext_grammar (Inference.search (fuzzer_oracle subst (Some goal) oracle_fname) g goal start max_depth forbidden_chars sgraph_fname qgraph_channel)
...
let fuzzer (complexity: int) (g: grammar) : string option = Option.map Grammar.string_of_word (Tree_fuzzer.fuzzer complexity None None g)
...
let read_bnf_grammar : bool -> string -> grammar = Grammar_io.read_bnf_grammar
let read_tokens : bool -> string -> element list = Grammar_io.read_tokens
let read_token : bool -> string -> element = Grammar_io.read_token
