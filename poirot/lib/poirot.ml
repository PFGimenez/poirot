...
let search (values: (element,string) Hashtbl.t option) (oracle_fname: string)  (g: grammar) (goal: element) (start: element list option) (max_depth: int) (forbidden: char list) (sgraph_fname: string option) (qgraph_channel: out_channel option) : grammar option =
    let fuzzer_oracle (values: (element,string) Hashtbl.t option) (goal: element option) (oracle_fname: string) : (grammar -> Oracle.oracle_status) = let oracle = Oracle.oracle_mem_from_script oracle_fname in (* create the oracle first because it is memoized *)
