...
let export_antlr4 (fname: string) (g: grammar) : unit =
    ...
    let export_antlr4_string (f: char -> string) (s: string) : string =
        (string_of_list "" "" f (List.init (String.length s) (String.get s))) in

    let export_antlr4_elem (e: element) : string = match e with
        | Nonterminal s -> String.lowercase_ascii (export_antlr4_string export_antlr4_char_nonterminal s)
        | Terminal s -> "'" ^ (export_antlr4_string export_antlr4_char_terminal s)  ^ "'" in
    ...
    let export_antlr4_rules (e: element): string =
        (String.lowercase_ascii (export_antlr4_string export_antlr4_char_nonterminal (string_of_element e)))^" : "^(string_of_list " | " "" export_antlr4_rhs (get_all_rhs g.rules e))^";\n" in
    ...
    output_string channel ("axiom : "^(String.lowercase_ascii ((export_antlr4_string export_antlr4_char_nonterminal (string_of_element g.axiom))))^" EOF;\n");
