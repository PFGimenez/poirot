...
let search ?(heuristic: Inference.heuristic = Inference.COMPLICATED) ?(manual_stop: bool = false) ?(oneline_comment: string option = None) ?(dict: (element,string) Hashtbl.t option = None) ?(max_depth: int = 10) ?(max_steps: int = 1000) ?(forbidden_chars: char list = []) ?(sgraph_fname: string option = None) ?(qgraph_fname: string option = None) ?(save_h: bool = true) ?(save_oracle: bool = true) (oracle: Oracle.t) (g: grammar) (goal: element) (start: element list) : (grammar * string) option =
    ...
    match Inference.search oracle g goal start oneline_comment dict max_depth max_steps sgraph_fname qgraph_fname h_fname o_fname forbidden_chars manual_stop heuristic with
    | None -> None
